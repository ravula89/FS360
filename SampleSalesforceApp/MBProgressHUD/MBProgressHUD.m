...
+ (MBProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	...
	[hud show:animated];
	 ...
	 NSLog(@"window subviews are %@",[window subviews]);
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	 ...
	 MBProgressHUD *hud = [MBProgressHUD HUDForView:(UIView *)[[window subviews] objectAtIndex:0]];//[MBProgressHUD HUDForView:view];
	if (hud != nil) {
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
		...
		self.labelText = @"Syncing ";
		self.detailsLabelText = @"In Process";
		self.opacity = 0.8f;
		 self.color = nil;
